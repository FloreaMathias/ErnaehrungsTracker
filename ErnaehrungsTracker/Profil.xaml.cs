...
namespace ErnaehrungsTracker
{
    public partial class Profil : Window
    {
        ...
        {
            MainWindow mainWindow = new MainWindow(goalWeight, currentWeight, profileName, 0);
            mainWindow.Show();
            Close();
        }
        ...
        {
            if(double.TryParse(ProfilWeightTextBoxINPUT.Text, out double weightToRemove))
            {
                weightCounter -= weightToRemove;
                kgremain.Text = weightCounter.ToString("0.0");
            }
            else
            {
